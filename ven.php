...
.navbar-light .navbar-brand:hover {
  ...
  <div id="main" style="margin-top:0px;">
        ...
        <span style="font-size:30px;cursor:pointer; margin-top:60%;" onclick="openNav()" >&#9776; Vendors</span>
  <table class="table" id="vendortable" style="width:100%;">
      ...
      db.collection("vendor").get().then((querySnapshot)=>{
        querySnapshot.forEach((doc)=>{
         ...
         var data = doc.data();
          console.log(data);
         var id = doc.id;
          document.getElementById("vendortable").innerHTML += '<tr><th scope="row">'+count+'</th><td>'+data.name+'</td><td>'+id+'</td><td>'+data.loc+'</td><td><button type="button" onclick="removevendor(event)" class="btn" style="color: black; background-color: lightgrey; border-style:solid; border-color: lightgrey; border-radius:0px; "><span class="glyphicon glyphicon-remove"></span> Remove</button></td></tr>'
